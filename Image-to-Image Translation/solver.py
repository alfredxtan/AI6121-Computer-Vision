...
def unnormalize(image):
    """
    Reverses the normalization on an image.

    Parameters:
    image (numpy.ndarray): The normalized image array.

    Returns:
    numpy.ndarray: The unnormalized image array.
    """
    mean = 0.5
    std = 0.5
    ...
    def train(self):
        ...
        for step in range(self.train_iters+1):
            ...
            if (step+1) % self.sample_step == 0:
                ...
                # svhn = inverse_normalize(torch.from_numpy(svhn),(0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
                # fake_svhn = inverse_normalize(torch.from_numpy(fake_svhn),(0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
                ...
                # im.save(path)
                #imageio is new
                #imageio.imwrite(path, Image.fromarray((merged * 255).astype(np.uint8)))
                merged = (merged + 1) / 2  # normalize to [0, 1]
